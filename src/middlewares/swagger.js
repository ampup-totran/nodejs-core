...
import schemas from 'utils/swagger-model';
...
<head>
  <meta charset="UTF-8">
  <title><% title %></title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/3.31.1/swagger-ui.min.css">
  <% favIconString %>
  <% customJs %>
  <style>
    html
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/3.31.1/swagger-ui-bundle.js"> </script>
...
function setup(swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle) {
  return async function (req, res) {
    if (!swaggerDoc) {
      const apiFiles = await getAPIFiles('./src/ddd/**/**.js');
      const swaggerOptions = {

        swaggerDefinition: {
          openapi: '3.0.0',
          info: {
            title: 'Provision Tool APIs',
            description: 'Provide APIs for ampUp\'s Provision Tool',
            version: '1.0.0',
            contact: {
              name: 'ampUp',
              email: 'help@ampup.io'
          ...
          components: {
            securitySchemes: {
              Bearer: {
                type: 'http',
                scheme: 'bearer',
                description: 'Your bearer token in the format **Bearer &lt;token>**'
              }
            },
            schemas
          },
          servers: [ {
            url: `{protocol}://${process.env.SWAGGER_API_URL}`,
            variables: {
              protocol: {
                enum: [ 'http', 'https' ],
                default: process.env.UP_STAGE === 'localhost' ? 'http' : 'https'
              }
            }
          } ]
        },
        apis: apiFiles || [ './build/ddd/status.js' ]
      };
...
function getAPIFiles(pathPattern, options = {}) {
  return new Promise((resolve, reject) => {
    glob(pathPattern, options, function (error, files) {
      if (error) {
        reject(files);
      }
      resolve(files);
...
function generateHTML(swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customSiteTitle, _htmlTplString, _jsTplString) {
  ...
  customCss = customCss || explorerString;

  customfavIcon = customfavIcon || 'https://ampup-public-assets.s3-us-west-2.amazonaws.com/ico/favicon.png';
  customSiteTitle = customSiteTitle || 'ampUp APIs Documentation';
...
function swaggerAssetMiddleware(options) {
  try {
    let opts = options || {};
    opts.index = false;
    return serveStatic(swaggerUi.getAbsoluteFSPath(), opts);
  } catch (error) {
    printLogConsole('[Error] - swaggerAssetMiddleware', error.message);
  }
}
